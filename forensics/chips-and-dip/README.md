...
* [chips-and-dip.pcapng](chips-and-dip.pcapng)

## Solution

This PCAP contains a number of HTTP requests sent back and forth between a computer that had been infected with malware as well as the C2 server controlling it. Examining the connections, you'll see that there is a request to "/system_update" first and then a downloaded excutable.

Examining the executable, we see that the malware generates a random 32 byte key, then encrypts it with an RSA public key embedded in it and transmits the message to the C2 server. Then it get back a ClientId for further communications. Next it begins a loop of requesting commands, executing them, and sending back the response however all of the communication is encrypted with Salsa20 with a key which we no longer have. Or we do, but it is encrypted with RSA in the begining registration sequence in the PCAP.

The key to this entire thing is examining the RSA public key and realizing that it is VERY short (only 256 bits). This means it can be effectively factored in about 2-3 minutes using a program like Yafu to find the orignal prime factors.

Once these are recovered then we are able to get back the private key, decrypt the registration request and get the 32 byte session key, then decrypt all further communications to reveal the commands that were sent including the last one asking the client to "cat /flag.txt".
